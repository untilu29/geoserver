...
RUN wget -O $CRYPTO_HOME https://git.axonvibelabs.com/devops/crypto/raw/master/crypto

ENV USER 1001
RUN chown -R 1001:0 "$GEOSERVER_HOME" && chmod -R ug+rwx "$GEOSERVER_HOME"
RUN chown 1001:0 "$CRYPTO_HOME" && chmod ug+rwx "$CRYPTO_HOME"
USER 1001
...
CMD  export AV_ASYMMETRIC_KEY=`cat /etc/cryptic/asym-key` && \
     ...
     crypto ; /opt/geoserver/bin/startup.sh
